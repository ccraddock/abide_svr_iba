...
for (roi in 1:200)
{
    ...
    sum=ddply(pred_df,.(diag_dir),summarize,roi=median(roi),mean=mean(r_std,na.rm=TRUE),sd=sd(r_std,na.rm=TRUE))
    if(roi == 1)
    {
        pred_df_sum=sum
    }
...
for (roi in 1:200)
{
    ...
    sum=ddply(rep_df,.(diag_dir),summarize,roi=median(roi),mean=mean(r_std,na.rm=TRUE),sd=sd(r_std,na.rm=TRUE))
    if(roi == 1)
    {
        rep_df_sum=sum
    }
