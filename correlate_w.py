...
def calc_corrs(roi):
    ...
    for test_sub in subids:
        real_file='/abide_data/data/%s/%s_w_%s.nii.gz'%(test_sub,test_sub,roi)
        print "working on %s %s"%(test_sub, real_file)
        try:
            real = nb.load(real_file).get_data()
            real = real.flatten()
        except IOError:
        ...
        for train_sub in subids:
            if train_sub not in test_sub:
                pred_file='/abide_data/data/%s/%s_w_%s.nii.gz'%(train_sub,train_sub,roi)
                try:
                    pred = nb.load(pred_file).get_data()
                    pred = pred.flatten()
                except IOError as e:
    ...
    with open("%s_rep_stats.csv"%(roi),"w") as outfd:
